...
class t_mat:
    ...
    def Self_Energy(self):
        ...
        Pair_Prop_11_12 = np.divide(U_mat, Den1)
        Pair_Prop_21_22 = np.divide(W_mat, Den1)
        Pair_Prop_12_SE = np.divide(U_mat, Den2)
        Pair_Prop_22_SE = np.divide(W_mat, Den2)

        IMat = np.eye(dim)

        # Dividing over M missing
        ...
        T22diag = np.diag(T_22_SE)
        for lambda_ in range(1, cutoff + 1):
            for kx in range(M):
                for ky in range(M):
                    indk = lambda_ * M * M + kx * M + ky
                    Sigma22 += T22diag[indk]
        indk0 = 0
        indq0 = 0
        sigpol = T_11[indk0, indq0] + T_12[indk0, indq0] + T_21[indk0, indq0] + T_22[indk0, indq0] + Sigma22
